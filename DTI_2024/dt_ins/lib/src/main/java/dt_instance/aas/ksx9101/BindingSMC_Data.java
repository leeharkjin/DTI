...
import java.util.List;
public class BindingSMC_Data {
	
	public static final Logger log = LoggerFactory.getLogger(BindingSMC_Data.class);
...
//	private static final String DEF_TBL_COL_MODIFIERID = "MODIFIERID";
	...
	private static final String TABLE_SUBPREFIX = "DATAINFO_";
...
//	
	...
	public BindingSMC_Data(DefaultSubmodelElementCollection value, BindingType type, DefaultReference refelement) {
		this();
		this._value = value;
		this._type = type;
...
//				break;
//			case "Equipment":
//				List<SubmodelElement> submods = new ArrayList<>(models.getValues());
//				SubmodelElement se = submods.get(0);
...
//		}
		...
		if (modelName.contains("EquipmentParameters"))
			modelName = "EquipmentParameter";
		else if (modelName.contains("EquipmentParameterValues"))
			modelName = "EquipmentParameterValues";
...
//		BindingTableSchema.appendKeysFromSchema(tableName, _data);
		...
		for (int i=0,l=submods.size(); i<l; i++) {
			...
			//리스트를 가져와서 전부 파싱해야함
			if (se instanceof SubmodelElementList) {
				DefaultSubmodelElementList datalist = (DefaultSubmodelElementList)se;
				int ende = datalist.getValue().size();
				for(int k=0; k<datalist.getValue().size();k++)
				{
					subse = datalist.getValue().get(k);
					
					subse.setIdShort(se.getIdShort());
					
					new BindingSMC_Data(
						(DefaultSubmodelElementCollection)subse,
						_type,
						this._DefaultReference // 여기에는 모델타입과 아이디가 들어가야됨
					).execute();
				}
...
//		_repo.set("SUBMODELID", _submodelid);
		...
		Set<String> tmp = BindingTableSchema.getColumnNames(tableName);
		int EVENTDATETIMEtag = 0;
		for (String element : tmp) {
		    
			try {
			   Object test = _data.get(element); // KeyNotFoundException 발생 가능
			   if(test == null)
			   {
				   _data.put(element, ""); // 키를 추가할 수 있음
				    if(element.equals("EVENTDATETIME"))
				    {
				    	EVENTDATETIMEtag=1;
				    }
			   }
		...
		if(EVENTDATETIMEtag==1) {
			...
			if(_data.get("EVENTDATETIME").equals(""))
				_data.put("EVENTDATETIME", currentTime);
		}
		
		//테이블 벨리데이션 처리 해주면 좋겠다 없는거 채우기 시도
