...
import java.util.List;
public class BindingSMC {
	
	public static final Logger log = LoggerFactory.getLogger(BindingSMC.class);
...
//	
	...
	public BindingSMC(DefaultSubmodelElementCollection value, BindingType type, DefaultReference refelement) {
		this();
		this._value = value;
		this._type = type;
...
//	}
	...
	public void execute() throws SQLException  {
		...
		switch (_submodelinfo) {
			...
			case "DataInfo":
			{
				...
				for (int i=0,l=submods.size(); i<l; i++) {
					...
					SubmodelElement se = submods.get(i);
		...
		{
			if(keylistsize>2)
				modelName = _refelement[keylistsize-2];
			else
			{
				log.debug("FA3ST.submodel : Index -1 out of bounds \n");
				return;
			}
		...
		submodelinfo = _refelement[1];
		
		if(submodelinfo == null)
		{
			log.warn("submodelinfo null 아직 정의 안됨.");
			return;
		}
		

		
		String tableName = TABLE_PREFIX + submodelinfo.toUpperCase() +"_" + modelName.toUpperCase();
